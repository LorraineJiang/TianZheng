...
public class Tonemapping : PostEffectsBase
{
	...
	public float UpdateCurve()
	{
		...
		if (remapCurve != null)
		{
			...
			for (float i = 0.0f; i <= 1.0f; i += 1.0f / 255.0f)
			{
				...
				curveTex.SetPixel(Mathf.FloorToInt(i * 255.0f), 0, new Color(c, c, c));
			}
			curveTex.Apply();
		}
		return 1.0f / range;
	...
	public void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		float downsample = Mathf.Log(rtSquared.width * 1.0f, 2);

		int div = 2;
		RenderTexture[] rts = new RenderTexture[(int)downsample];
		...
		var lumRt = rts[(int)downsample - 1];
		Graphics.Blit(rtSquared, rts[0], tonemapMaterial, 1);
		if (type == TonemapperType.AdaptiveReinhardAutoWhite)
		{
			for (int i = 0; i < (int)downsample - 1; i++)
			{
		...
		for (int i = 0; i < (int)downsample; i++)
		{
