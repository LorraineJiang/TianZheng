...
public class bl_MiniMapEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (EditorGUILayout.BeginFadeGroup(GeneralAnim.faded))
        {
            ...
            script.isHideButtonInGlobal = EditorGUILayout.ToggleLeft("IsHideButtonInGlobal", script.isHideButtonInGlobal, EditorStyles.toolbarButton);
        }
        ...
        if (EditorGUILayout.BeginFadeGroup(ZoomAnim.faded))
        {
            ...
            EditorGUILayout.MinMaxSlider(ref script.MinZoom, ref script.MaxZoom, 1, 1000);
        ...
        if (EditorGUILayout.BeginFadeGroup(DragAnim.faded))
        {
            ...
            if (script.CanDragMiniMap)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    v.x = EditorGUILayout.Slider("Horizontal Speed", v.x, 0.01f, 100);
                    v.y = EditorGUILayout.Slider("Vertical Speed", v.y, 0.01f, 100);
                    script.DragMovementSpeed = v;
                }

                Vector2 maxv2 = script.MaxOffSetPosition;
                Vector2 minV2 = script.MinOffSetPosition;
                using (new EditorGUILayout.VerticalScope())
                {
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        minV2.x = EditorGUILayout.Slider("Min Horizontal", minV2.x, 1, 5000);
                        maxv2.x = EditorGUILayout.Slider("Max Horizontal", maxv2.x, 1, 5000);
                    }
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        maxv2.y = EditorGUILayout.Slider("Max Vertical", maxv2.y, 1, 5000);
                        minV2.y = EditorGUILayout.Slider("Min Vertical", minV2.y, 1, 5000);
                    }
                }
                script.MaxOffSetPosition = maxv2;
                script.MinOffSetPosition = minV2;
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    v3.x = EditorGUILayout.Slider("Cursor X offset", v3.x, 0.01f, 20);
                    v3.y = EditorGUILayout.Slider("Cursor Y offset", v3.y, 0.01f, 20);
                    script.HotSpot = v3;
                }
            }
        }
        ...
        if (EditorGUILayout.BeginFadeGroup(ReferencesAnim.faded))
        {
            ...
            script.mapButtons= EditorGUILayout.ObjectField(new GUIContent("Map Buttons","the minimap buttons"), script.mapButtons, typeof(GameObject), allowSceneObjects) as GameObject;
            script.isShowCompass = EditorGUILayout.ToggleLeft(new GUIContent("Show MapCompass","show compass when map is global"), script.isShowCompass, EditorStyles.toolbarButton);
        }
        EditorGUILayout.EndFadeGroup();
        EditorGUILayout.EndVertical();
