...
public class bl_MiniMapItem : MonoBehaviour {
    ...
    //the icon button callback
    public UnityAction clickCallBack;
    public bool isShowItemName;
    //Privates
    ...
    void CreateIcon()
    {
        ...
        if (RectRoot.GetComponentInChildren<bl_MaskHelper>() != null)
            cacheItem.transform.SetParent(RectRoot.GetComponentInChildren<bl_MaskHelper>().transform, false);
        else
        ...
        {
            cacheItem.GetComponent<Button>().onClick.AddListener(clickCallBack);
        }
