...
public class bl_MiniMap : MonoBehaviour
{
    ...
    [Tooltip("Hide MapButton when world size")]
    public bool isHideButtonInGlobal;
    ...
    public Vector2 MinOffSetPosition = new Vector2(1000, 1000);
    ...
    void Inputs()
    {
        ...
        if (Input.GetKey(DecreaseHeightKey) && DefaultHeight < MaxZoom)
        {
            ChangeHeight(true);
        }
        if (Input.GetKey(IncreaseHeightKey) && DefaultHeight > MinZoom)
        {
            ChangeHeight(false);
        }
    }
    ...
    public void ToggleSize()
    {
        ...
        if(this.isShowCompass)
            GetComponent<bl_MMCompass>()?.CompassRoot.gameObject.SetActive(true);
    }
    ...
    public void SetDragPosition(Vector3 pos)
    {
        ...
        DragOffset.x = Mathf.Clamp(DragOffset.x, -MinOffSetPosition.x - m_Target.transform.position.x, MaxOffSetPosition.x - m_Target.transform.position.x);
        DragOffset.z = Mathf.Clamp(DragOffset.z, -MinOffSetPosition.y - m_Target.transform.position.z, MaxOffSetPosition.y - m_Target.transform.position.z);
    }
    ...
    {
        if (!AllowMultipleMarks && mapPointer != null)
            Destroy(mapPointer);
    }
    ...
    /// </summary>
    /// <param name="item">iteminformation</param>
    /// <returns></returns>
    ...
    /// </summary>
    /// <param name="item">iteminformation</param>
    /// <param name="itemName">the itemicon name</param>
    /// <param name="isShowName">show name when create</param>
    /// <param name="callback">clickcallback</param>
    /// <returns></returns>
    public bl_MiniMapItem CreateNewItem(bl_MMItemInfo item, string itemName = "", bool isShowName = false, UnityAction callback = null)
    {
        if (hasError) return null;
        ...
        mmItem.m_Effect = item.Effect;
        if (item.Sprite != null) { mmItem.Icon = item.Sprite; }
        mmItem.clickCallBack = callback;
        mmItem.InfoItem = itemName;
        mmItem.isShowItemName = isShowName;
        return mmItem;
    }
